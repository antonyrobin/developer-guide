...
<p><mark>Highlighted</mark> and <small>fine print</small>.</p>`,
    ...
    },
    {
      ...
      content: `In HTML, \`<div>\` and \`<span>\` are **generic containers**. They don't have inherent meaning (like \`<p>\` or \`<h1>\`), but they are essential for grouping content to apply styles or layout structures using CSS.\n\n### Block-Level: div\n\n\`<div>\` is a **Block-level element** — think of it as a "box" that takes up the **full width** of the page. It always starts on a **new line** and pushes following elements below. *Use Case:* Creating layouts, sections, or wrapping large groups of elements.\n\n### Inline: span\n\n\`<span>\` is an **Inline element** — think of it as a "marker" that only wraps as much width as the text inside. It does *not* start on a new line. *Use Case:* Styling a specific word or phrase inside a paragraph.\n\n### Key Differences\n\n**Block elements** are the "bricks" of your webpage — they always start on a new line, stretch to fill **100% width**, and you can freely set \`width\`, \`height\`, \`padding\`, and \`margin\`. They can contain other block or inline elements (e.g., \`<div>\`, \`<p>\`, \`<h1>\`).\n\n**Inline elements** are like "words" in a sentence — they stay on the same line, take up only as much width as their content, and you **cannot** set \`width\` or \`height\` on them. Use \`display: inline-block\` to get both inline flow *and* dimension control.`,
      image: '/images/html/html-block-inline.svg',
...
<span style="background: lightgreen;">
  I sit next to the span above.
    ...
    },
    {
      ...
      content: `In web design, links and navigation are the "connective tissue" that turns a collection of isolated pages into a functional website. The fundamental building block is the **anchor tag** \`<a>\` — by default an inline element.\n\n### Link Syntax\n\n\`<a href="URL">Click Me</a>\`. Key attributes: \`href\` (destination URL), \`target="_blank"\` (opens in new tab), \`title\` (tooltip on hover). When using \`target="_blank"\`, **always** add \`rel="noopener noreferrer"\` for security to prevent "tabnapping."\n\n### Link Types\n\nLinks can be **absolute** (full URL: \`https://google.com\`) or **relative** (path within your site: \`/about.html\`). Relative links are preferred for internal navigation. **Fragment links** (\`href="#section-id"\`) scroll to matching elements on the same page. **Tel links** (\`href="tel:+1234567890"\`) and **email links** (\`href="mailto:name@example.com"\`) trigger phone calls and email clients.\n\n### Navigation Bars\n\nFor professional navigation, wrap links in a \`<ul>\`/\`<li>\` list inside a \`<nav>\` container. Then use CSS: \`list-style: none\` to remove bullets, \`display: flex\` on \`<ul>\` for horizontal alignment, and \`display: block\` on \`<a>\` tags to make the entire button area clickable.\n\n**Best Practices:** Keep the menu in the same place on every page (*Consistency*). Use simple labels like "Contact" not "Reach Out" (*Clarity*). Use \`:hover\` and \`:active\` CSS states for feedback.`,
...
<a href="mailto:info@example.com">Email Us</a>`,
    ...
    {
      title: 'Images & Lists',
      content: `The \`<img>\` tag embeds an image — it is **self-closing** (no \`</img>\` needed). Key attributes: \`src\` (image path), \`alt\` (alternative text for **accessibility** and **SEO**), \`width\`/\`height\` (dimensions). **Best Practice:** Always include \`alt\` to avoid accessibility issues. For decorative images, use \`alt=""\`.\n\n### Figure & Figcaption\n\nFor professional semantic structure, use the \`<figure>\` element to group images with captions: \`<figure>\` wraps the \`<img>\`, and \`<figcaption>\` provides the caption.\n\n### Lists\n\n**Unordered Lists** (\`<ul>\`) are for items where sequence doesn't matter — they default to bullet points. Common uses: navigation menus, feature lists, shopping lists. **Ordered Lists** (\`<ol>\`) are for sequential items — they default to numbers. You can use \`type="a"\` for letters, \`type="i"\` for Roman numerals, or \`reversed\` to count down.\n\n### Nesting Lists\n\nYou can put a list inside another list item (\`<li>\`). This is how **dropdown menus** and complex outlines are built. *Crucial Rule:* When nesting, the new list must go **inside** the \`<li>\` tag of the parent, not between them. Correct: \`<li>Fruit <ul>...</ul></li>\`. Incorrect: \`<li>Fruit</li> <ul>...</ul>\`.`,
...
<!-- This comment is invisible in the browser -->`,
    ...
    {
      title: 'Tables',
      content: `**Tables** in HTML organize complex data into a grid of rows and columns. While they were used for page layouts in the 1990s, today they should **only** be used for *tabular data* (like spreadsheets, schedules, or pricing grids).\n\n### Core Table Structure\n\nTables are built **row by row**: \`<table>\` is the wrapper, \`<tr>\` (*Table Row*) defines a horizontal row, \`<th>\` (*Table Header*) defines a header cell (**bold and centered** by default), and \`<td>\` (*Table Data*) defines a standard content cell.\n\n### Semantic Grouping\n\nFor proper structure, use \`<thead>\` for header rows, \`<tbody>\` for data rows, and \`<tfoot>\` for footer/summary rows. The \`<caption>\` element provides a **title** for the table — important for both SEO and accessibility.\n\n### Accessibility\n\nUse the \`scope\` attribute on \`<th>\` to specify whether it heads a column (\`scope="col"\`) or a row (\`scope="row"\`). **Screen readers** rely on this to navigate table data properly. See the [MDN Table Accessibility Guide](https://developer.mozilla.org/en-US/docs/Learn/HTML/Tables/Advanced) for more.\n\n### Merging Cells\n\nThe \`colspan\` and \`rowspan\` attributes allow cells to span multiple columns or rows, creating **merged cells** for complex layouts.`,
  ...
  <tfoot>
    ...
    },
    {
      ...
      content: `**Forms** are the primary way users interact with a website, allowing them to send data — searches, logins, contact info — to a server. The \`<form>\` tag acts as a container with two critical attributes: \`action\` (the URL where data is sent) and \`method\` (how data is sent — **GET** appends to URL for searches, **POST** hides in request body for passwords/private info).\n\n### Common Form Elements\n\n\`<input type="text">\` for single-line text, \`<input type="password">\` to mask characters, \`<input type="checkbox">\` for multiple selections, \`<input type="radio">\` for single selection from a group (must share the same \`name\`), \`<textarea>\` for multi-line text, and \`<select>\` with \`<option>\` tags for dropdown menus.\n\n### Labels & Validation\n\nEach input needs a \`<label>\` linked via the \`for\`/\`id\` pair — this improves **usability** (clicking label focuses input) and is essential for **screen readers**. HTML5 provides *declarative validation*: \`required\` (prevents empty submission), \`placeholder\` (hint text), \`minlength\`/\`maxlength\` (character limits), and \`pattern\` (regex — e.g., \`pattern="[0-9]{3}"\` for 3 digits).\n\n### Advanced Input Types\n\n\`email\` (checks for @), \`date\` (calendar picker), \`range\` (slider with min/max), \`color\` (color picker), \`tel\` (numeric keypad on mobile). The \`<datalist>\` element creates a *combo box* — users can type to filter suggestions. Use \`<fieldset>\` and \`<legend>\` to group related fields, and \`<output>\` for real-time calculation results. See the [MDN Form Guide](https://developer.mozilla.org/en-US/docs/Learn/Forms) for more.`,
      image: '/images/html/html-form-inputs.svg',
  ...
  <fieldset>
    ...
    },
    {
      ...
      content: `**Semantic elements** solve the problem of meaningless \`<div>\` soup by using tags that clearly describe their meaning. They behave exactly like a \`<div>\` (block-level), but carry extra information for **search engines** and **screen readers**.\n\n### Key Semantic Tags\n\n\`<header>\` — Contains introductory content, the logo, and navigation. \`<nav>\` — Reserved specifically for the **main navigation** links. \`<main>\` — Holds the unique, central content of the page (*only one* \`<main>\` per page). \`<article>\` — A self-contained piece of content that could make sense on its own (blog post, forum post, news story). \`<section>\` — Groups related content together (e.g., "Features" or "About Us"). \`<aside>\` — Content indirectly related to the main piece (sidebar, pull-quotes, advertising). \`<footer>\` — The bottom of the page containing copyright info, contact links, or site maps.\n\n### Semantic vs. Non-Semantic\n\nInstead of \`<div id="header">\`, use \`<header>\`. Instead of \`<div class="nav">\`, use \`<nav>\`. Instead of \`<div class="post">\`, use \`<article>\`. **Benefits:** Better **SEO** (search engines understand structure), **accessibility** (screen readers navigate by landmarks), and **maintainability** (self-documenting code).\n\n### Additional Semantic Elements\n\nOther useful elements include \`<time>\` (specific dates/times), \`<address>\` (contact information), \`<details>\` and \`<summary>\` (collapsible content), and \`<dialog>\` (modal windows). Learn more at the [MDN Semantic Elements Reference](https://developer.mozilla.org/en-US/docs/Glossary/Semantics#semantics_in_html).`,
      image: '/images/html/html-semantic-layout.svg',
      code: `<body>
  <header>
    <nav>
      <ul>
  ...
  <footer>
    ...
    },
    {
      ...
      content: `**Multimedia** combines different content forms to deliver a dynamic, engaging experience. HTML5 introduced **native multimedia support** — no plugins needed.\n\n### Audio\n\nThe \`<audio>\` element supports attributes: \`controls\` (player UI), \`autoplay\` (auto-play), and \`loop\`. Provide multiple \`<source>\` tags for **cross-browser compatibility**. Audio types include *speech/voiceover* (narration), *music* (mood and pace), and *sound effects* (UI feedback like a "click" on button press).\n\n### Video\n\n**Video** is the most powerful multimedia element — it combines text, audio, images, and movement. The \`<video>\` element uses: \`controls\` for play/pause/volume, \`poster\` for a **preview image** before playback, and \`width\`/\`height\` for dimensions. The \`<track>\` element adds **subtitles** via \`.vtt\` files for accessibility.\n\n### Responsive Media\n\nThe \`<figure>\` and \`<figcaption>\` elements semantically group any media with captions. For responsive images, use \`srcset\` to provide multiple sizes — the browser automatically selects the best version. The \`<picture>\` element provides *Art Direction* — showing different images for different screen sizes (e.g., vertical crop for mobile, wide shot for desktop). See the [MDN Responsive Images Guide](https://developer.mozilla.org/en-US/docs/Learn/HTML/Multimedia_and_embedding/Responsive_images).`,
...
<picture>
  ...
  <img src="mobile-hero.jpg" alt="Hero banner">
    ...
    },
    {
      ...
      content: `An \`<iframe>\` (*Inline Frame*) is like a window cut into your webpage that displays another page. It allows you to embed **external content** — YouTube videos, Google Maps, third-party widgets — without the user leaving your domain.\n\n### Security\n\nSecurity is the biggest concern since you're running someone else's code inside your site. The \`sandbox\` attribute is your primary defense — it applies restrictions to embedded content. Options include: \`allow-scripts\` (re-enables JavaScript), \`allow-forms\` (allows form submission), \`allow-same-origin\` (retains origin claims for cookies/storage), and \`allow-popups\` (permits new windows). **Pro Tip:** Always use the most restrictive sandbox possible. For a static map, leave sandbox empty: \`<iframe sandbox src="...">\`.\n\n### Performance & Modern Attributes\n\n\`loading="lazy"\` tells the browser not to load the iframe until the user scrolls near it — significantly boosts **page speed**. \`referrerpolicy\` controls how much referral info is sent to the external site. The \`allow\` attribute is used for *Feature Policy* — granting access to camera, microphone, or fullscreen. See the [MDN iframe Reference](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/iframe) for full details.`,
      code: `<!-- Basic iframe -->
...
<iframe width="560" height="315"
    ...
    },
    {
      ...
      content: `The \`<canvas>\` element is a powerful **blank slate** for drawing graphics via JavaScript. Unlike retained-mode HTML elements, Canvas is an *immediate-mode* system — you tell the browser what to paint, pixel by pixel, and it forgets immediately.\n\n### Getting Started\n\nDefine the element in HTML and access its **context** in JavaScript via \`getContext('2d')\`. The coordinate system starts at the top-left corner (0,0): *X increases rightward*, *Y increases downward*.\n\n### Core Drawing Techniques\n\n**Basic Shapes:** \`fillRect()\` draws solid rectangles, \`strokeRect()\` draws outlines, \`clearRect()\` acts as an eraser. **Complex Paths:** \`beginPath()\` starts a path, \`moveTo()\` lifts the "pen," \`lineTo()\` draws lines, \`arc()\` draws curves/circles, then \`fill()\` or \`stroke()\` renders it.\n\n### State & Animation\n\n\`ctx.save()\` pushes current state (color, rotation) onto a stack, \`ctx.restore()\` pops it back. For animation, use the **game loop pattern**: clear canvas → update positions → redraw → repeat with \`requestAnimationFrame()\` for smooth **60fps** motion.\n\n### SVG vs Canvas\n\n**SVG** (*Scalable Vector Graphics*) is XML-based and perfect for icons/logos — vector-based, resizes without quality loss. **Canvas** is best for complex dynamic graphics (games, data visualization). **SVG** is best for scalable UI elements (icons, charts, logos). See the [MDN Canvas Tutorial](https://developer.mozilla.org/en-US/docs/Web/API/Canvas_API/Tutorial).`,
...
<svg width="100" height="100">
  <circle cx="50" cy="50" r="40" stroke="black" fill="red" />
    ...
    },
    {
      ...
      content: `HTML5 introduced powerful **JavaScript APIs** accessible from HTML.\n\n### Geolocation API\n\nAllows users to share their physical location. The browser always asks for **explicit permission**. \`getCurrentPosition()\` is a one-time location request; \`watchPosition()\` tracks movement (for *navigation apps*).\n\n### Web Storage API\n\nBefore HTML5, only cookies existed (tiny, sent with every request). Web Storage stores much larger amounts locally. \`localStorage\` **persists** even after browser close. \`sessionStorage\` **clears** when the tab closes.\n\n### Drag and Drop API\n\nMakes any element draggable with \`draggable="true"\`. Involves events: \`dragstart\`, \`dragover\`, and \`drop\`.\n\n### Custom Data Attributes\n\n\`data-*\` attributes store extra information directly in HTML elements — a bridge between HTML and JavaScript without clashing with standard attributes. Access via \`element.dataset\` in JS (e.g., \`data-product-id\` becomes \`dataset.productId\`).\n\n### Templates & Entities\n\nThe \`<template>\` element creates **blueprints** of HTML that the browser parses but does **not render** until activated via JavaScript — perfect for reusable components. **HTML Entities** are special codes for reserved characters (\`&lt;\` for <, \`&gt;\` for >, \`&amp;\` for &, \`&nbsp;\` for non-breaking space) — they prevent code injection and display special symbols. See the [MDN Web APIs Reference](https://developer.mozilla.org/en-US/docs/Web/API).`,
...
<p>&copy; 2026 &nbsp; &mdash; All Rights Reserved</p>`,
    ...
    },
    {
      ...
      content: `Accessibility isn't just a "nice-to-have" — it's a **fundamental** part of the modern web. **ARIA** (*Accessible Rich Internet Applications*) is a set of attributes that help assistive technologies like screen readers understand what the page is doing.\n\n### Key ARIA Attributes\n\n\`role="button"\` defines the purpose of an element (for non-semantic elements acting as buttons). \`aria-label="Close"\` provides screen reader text when there is no visible label. \`aria-hidden="true"\` hides decorative elements from assistive tech. \`aria-live="assertive"\` announces dynamic content changes immediately.\n\n### Responsive Design\n\nThe **viewport meta tag** is critical: \`<meta name="viewport" content="width=device-width, initial-scale=1.0">\` — without it, mobile browsers assume a desktop-only site and zoom out, making text unreadably small.\n\n### Responsive Best Practices\n\nFor responsive images, avoid fixed widths (\`width="800"\`) in HTML attributes — use CSS \`max-width: 100%\` instead. Structure HTML with **semantic containers** (\`<header>\`, \`<main>\`, \`<section>\`, \`<footer>\`) so CSS can easily stack elements on mobile and spread them on desktop. Test accessibility with tools like [WAVE](https://wave.webaim.org/) and the [axe DevTools](https://www.deque.com/axe/) browser extension.`,
...
<main>
  <section>
    ...
    },
    {
      ...
      content: `**HTML Validation** is the process of checking your code against official [W3C standards](https://validator.w3.org/). Just because a website looks right doesn't mean the code is correct — browsers are incredibly forgiving and will try to "fix" broken HTML, often with **unpredictable results**.\n\n### Why Validate?\n\n**Browser Consistency** — your site looks the same across Chrome, Safari, Firefox, and Edge. **SEO** — search engine crawlers index content more accurately with clean structure. **Future-Proofing** — valid code remains compatible with future browser updates. **Accessibility** — screen readers rely on proper nesting and attributes.\n\n### Common Validation Errors\n\n*Unclosed Tags* — forgetting \`</div>\` breaks layout from that point forward. *Improper Nesting* — block element inside inline, e.g., \`<div>\` inside \`<p>\`. *Missing alt Attributes* — all images need \`alt\`, even if empty. *Duplicate IDs* — \`id\` must be **unique** per document. *Obsolete Tags* — like \`<center>\` or \`<font>\` — replaced by CSS.\n\n### Best Practices\n\nUse **semantic tags** over generic \`<div>\`. Optimize for mobile with the **viewport meta** tag. Indent code for readability. Place CSS in \`<head>\`, scripts at bottom of \`<body>\` with \`defer\`. Use \`<link rel="preload">\` for critical resources. Avoid inline JS — use *Content Security Policy* (CSP) for security. Minify HTML for production.`,
      keyPoints: [
    ...
    { title: 'Best Practices & Code Standards', content: `**Semantic HTML** is the foundation of accessible, maintainable web development. Always use the most meaningful element for your content — \`<article>\` for self-contained content, \`<nav>\` for navigation, \`<section>\` for thematic groups, and \`<button>\` for clickable actions (never \`<div onclick>\`).\n\n### Accessibility\n\n**Accessibility (a11y)** is not optional. Every \`<img>\` needs a meaningful \`alt\` attribute (use \`alt=""\` for purely decorative images). All form inputs must have associated \`<label>\` elements. Use ARIA attributes only when native HTML semantics are insufficient — native elements like \`<button>\`, \`<dialog>\`, and \`<details>\` already have built-in accessibility.\n\n### Document Structure\n\nMaintain a logical **heading hierarchy** — never skip levels (go \`<h1>\` to \`<h2>\` to \`<h3>\`). Every page should have exactly one \`<h1>\`. Use \`<meta>\` tags for viewport, charset, and description. Validate your HTML with the [W3C Validator](https://validator.w3.org/) to catch structural errors early.`, code: `<!-- DO: Semantic HTML -->\n<header>\n  <nav aria-label="Main navigation">\n    <a href="/">Home</a>\n    <a href="/about">About</a>\n  </nav>\n</header>\n<main>\n  <article>\n    <h1>Page Title</h1>\n    <p>Content here...</p>\n  </article>\n</main>\n<footer>Contact info</footer>\n\n<!-- DON'T: Div soup with no meaning -->\n<div class="header">\n  <div class="nav">\n    <div onclick="goHome()">Home</div>\n  </div>\n</div>\n\n<!-- DO: Accessible images and forms -->\n<img src="chart.png" alt="Sales increased 40% in Q4">\n<img src="decoration.svg" alt="">\n\n<label for="email">Email</label>\n<input id="email" type="email" required>\n\n<!-- DON'T: Missing labels and alt text -->\n<img src="chart.png">\n<input type="email" placeholder="Email">`, codeLabel: 'Dos & Don\'ts', keyPoints: ['Always use semantic elements over generic divs and spans.', 'Every image needs meaningful alt text (empty alt="" for decorative).', 'Never skip heading levels — go h1, h2, h3 in order.', 'Validate HTML with W3C validator to catch structural errors.'] }
  ]
};
