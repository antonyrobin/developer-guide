export const jsCourse = {
  ...
  sections: [
    ...
    },
    {
      ...
      content: `### Variable Declarations\n\nJavaScript has three ways to declare variables. \`let\` declares a **block-scoped** variable that can be reassigned. \`const\` declares a **block-scoped** variable that cannot be reassigned (but objects/arrays it holds CAN be mutated). \`var\` is the legacy keyword — it is **function-scoped** and hoisted, leading to subtle bugs. Rule: always use \`const\` by default, use \`let\` when you need to reassign, never use \`var\`.\n\n### Data Types\n\nJavaScript has **8 data types**. **Primitive types** are immutable values passed by copy: [String](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String) (\`"hello"\`), [Number](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Number) (\`42\`, \`3.14\` — no separate integer type), [BigInt](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/BigInt) (very large integers), [Boolean](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Boolean) (\`true\`/\`false\`), \`undefined\` (declared but not yet assigned), \`null\` (intentionally empty), and [Symbol](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Symbol) (unique identifiers). **Reference types** are mutable and passed by reference: \`Object\` (\`{key: value}\`), \`Array\` (\`[1, 2, 3]\`), and \`Function\`.\n\n### Type Coercion\n\n**Type coercion** is JavaScript's automatic type conversion. It causes notorious quirks: \`"5" + 3\` = \`"53"\` (string concatenation) but \`"5" - 3\` = \`2\` (numeric subtraction). Explicit conversion: \`Number("5")\` → \`5\`, \`String(42)\` → \`"42"\`, \`Boolean(0)\` → \`false\`. Always use \`===\` (**strict equality**) instead of \`==\` (loose equality with coercion) to avoid surprises.\n\n### Scope & Hoisting\n\n\`var\` declarations are **hoisted** to the top of their function and initialized as \`undefined\`. \`let\` and \`const\` are hoisted but remain in a **"temporal dead zone"** until their declaration line — accessing them before causes a \`ReferenceError\`. [Function declarations](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/function) are fully hoisted including their body.`,
      image: '/images/javascript/js-data-types.svg',
...
const copy = { ...user, age: 30 }; // Clone + override`,
    ...
    },
    {
      ...
      content: `**Arrays** are ordered collections of values. Modern JavaScript provides powerful built-in methods that make array manipulation concise and expressive using a *functional, immutable* style.\n\n### Transformation Methods\n\nTransformation methods (return a **NEW array** — immutable): [\`map()\`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/map) transforms each element into a new value. [\`filter()\`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/filter) keeps only elements passing a test. \`slice(start, end)\` extracts a portion without mutating. \`flat()\` / \`flatMap()\` flattens nested arrays.\n\n### Search & Test Methods\n\n\`find()\` returns the first matching element (or \`undefined\`). \`findIndex()\` returns its index. \`includes()\` checks if a value exists. \`some()\` returns \`true\` if ANY element passes. \`every()\` returns \`true\` if ALL elements pass.\n\n### Reduction\n\n[\`reduce(callback, initialValue)\`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Array/reduce) accumulates all elements into a single value — the *Swiss Army knife* of array methods. It can replicate \`map\`, \`filter\`, and more.\n\n### Mutation Methods\n\n**Mutation methods** (modify original — use carefully): \`push()\`/\`pop()\` add/remove from end. \`unshift()\`/\`shift()\` add/remove from start. \`splice(index, count, ...items)\` inserts/removes at any position. \`sort()\` sorts in place — **WARNING**: sorts as strings by default, use a comparator for numbers: \`.sort((a, b) => a - b)\`. \`reverse()\` reverses in place.\n\n### Destructuring & Spread\n\nDestructuring and spread: \`const [first, ...rest] = array\`. \`[...arr1, ...arr2]\` merges. \`[...new Set(arr)]\` removes duplicates.`,
...
const unique = [...new Set([1, 2, 2, 3])]; // [1, 2, 3]`,
    ...
    },
    {
      ...
      content: `The [Document Object Model (DOM)](https://developer.mozilla.org/en-US/docs/Web/API/Document_Object_Model) is the browser's live, in-memory representation of an HTML page as a tree of objects. Every tag becomes a **node** in the tree. JavaScript can read, modify, add, or remove any node — making the page dynamic and interactive.\n\n### Selecting Elements\n\n\`getElementById("id")\` is fastest for single elements. \`querySelector(cssSelector)\` selects the first match for any CSS selector. \`querySelectorAll(cssSelector)\` returns a static \`NodeList\` of all matches.\n\n### Reading & Modifying\n\n\`element.textContent\` reads/sets text (safe from **XSS**). \`element.innerHTML\` reads/sets raw HTML — **never set with user input**. \`element.getAttribute("href")\` reads an attribute. \`element.setAttribute("src", "new.jpg")\` sets one. \`element.style.color = "red"\` sets inline styles. \`classList.add()\`, \`.remove()\`, \`.toggle()\`, \`.contains()\` manage CSS classes cleanly.\n\n### Creating & Inserting\n\n\`document.createElement("div")\` creates a new element. Set its properties, then insert with \`appendChild()\`, \`insertBefore()\`, or the modern \`prepend()\`/\`append()\`/\`after()\`/\`before()\`. \`element.remove()\` deletes an element.\n\n### Events\n\n[\`addEventListener("event", handler)\`](https://developer.mozilla.org/en-US/docs/Web/API/EventTarget/addEventListener) attaches events. Common events: \`click\`, \`input\`, \`submit\`, \`keydown\`, \`mouseover\`, \`scroll\`, \`DOMContentLoaded\`. **Event delegation** — attaching one listener to a parent for all children — is a key performance technique for dynamic lists.`,
      image: '/images/javascript/js-dom-tree.svg',
      code: `// Selecting
...
results.forEach(r => {
  if (r.status === "fulfilled") console.log(r.value);
    ...
    {
      title: 'Async / Await',
      content: `[\`async\`/\`await\`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/async_function) (introduced in **ES2017**) is syntactic sugar over **Promises** that makes asynchronous code look and behave like synchronous code. It is the modern standard for writing async JavaScript.\n\n### How It Works\n\nThe \`async\` keyword marks a function as async — it automatically returns a **Promise**. Inside an async function, you can use the \`await\` keyword before any Promise. The execution pauses at that point until the Promise settles, then continues with the resolved value. Critically, the browser is NOT blocked — other JavaScript can run while awaiting.\n\n### Error Handling\n\nWrap \`await\` calls in \`try\`/\`catch\` blocks to handle rejections cleanly. The \`catch\` block catches both network errors and manually thrown errors. The \`finally\` block always runs — use it to clean up (*hide loading spinners, close connections*).\n\n### Top-Level Await\n\n**Top-level await**: In **ES2022** modules, you can use \`await\` directly at the module's top level without an \`async\` wrapper. This is useful for *module initialization*.\n\n### Parallel Execution\n\nAwaiting in sequence (\`const a = await fetchA(); const b = await fetchB()\`) is **slow** — B only starts after A finishes. Use \`Promise.all\` for parallel: \`const [a, b] = await Promise.all([fetchA(), fetchB()])\`. If one can fail independently, use \`Promise.allSettled\`.\n\n### Debugging\n\n\`async\`/\`await\` integrates with browser **DevTools** far better than Promises — stack traces are clearer and stepping through code works naturally.`,
...
if (res.status === 429) console.log("Rate limited — retry later");`,
    ...
    },
    {
      ...
      content: `**JSON** (*JavaScript Object Notation*) is the universal data format for [REST APIs](https://developer.mozilla.org/en-US/docs/Glossary/REST). It is a lightweight text format based on JavaScript object syntax, readable by humans and easily parsed by machines.\n\n### JSON Types\n\nJSON supports exactly **six types**: strings (always double-quoted), numbers, booleans (\`true\`/\`false\`), \`null\`, arrays, and objects. It does **NOT** support: functions, \`undefined\`, comments, trailing commas, or single-quoted strings.\n\n### Serialization & Parsing\n\n[\`JSON.stringify(value, replacer, spaces)\`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/JSON/stringify) converts a JavaScript value to a JSON string. [\`JSON.parse(text, reviver)\`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/JSON/parse) parses a JSON string back to a JavaScript value. Both can throw errors — always wrap in \`try\`/\`catch\`.\n\n### API Response Patterns\n\nA typical REST API returns a response body structured as JSON. Common patterns include the **data envelope** \`{ "data": [...], "meta": {...}, "status": "success" }\`, **error envelope** \`{ "error": "Not found", "code": 404 }\`, and **pagination** \`{ "data": [...], "pagination": { "page": 1, "totalPages": 10 } }\`.\n\n### URL Anatomy\n\nA URL consists of **Protocol** (\`https://\`), **Host** (\`api.example.com\`), **Port** (\`:443\`), **Path** (\`/users/123\`), **Query String** (\`?page=1&limit=20\`), and **Fragment** (\`#section\`). In \`fetch()\`, build query strings with [\`URLSearchParams\`](https://developer.mozilla.org/en-US/docs/Web/API/URLSearchParams) for proper encoding.`,
...
async function getAllPosts() {
  ...
  return data;
    ...
    },
    {
      ...
      content: `**Web Storage** provides two APIs for storing data in the browser without sending it to the server on every request — a massive improvement over the original cookies.\n\n### localStorage\n\n[\`localStorage\`](https://developer.mozilla.org/en-US/docs/Web/API/Window/localStorage) is **persistent**: data stays even after the browser closes. It is scoped to the **origin** (protocol + domain + port). Max storage is typically **5–10 MB** depending on the browser. Great for *user preferences, theme settings, or cached data*.\n\n### sessionStorage\n\n[\`sessionStorage\`](https://developer.mozilla.org/en-US/docs/Web/API/Window/sessionStorage) is **session-scoped**: data is cleared when the browser tab is closed. Each tab gets its own \`sessionStorage\`. Great for *one-time form data, temporary authentication state, or wizard step data*.\n\n### API Interface\n\nBoth APIs have the same interface: \`setItem(key, value)\`, \`getItem(key)\` → \`string | null\`, \`removeItem(key)\`, \`clear()\`, and a \`length\` property. Important: both only store **strings**. Use \`JSON.stringify\` when storing objects and \`JSON.parse\` when reading them back.\n\n### Limitations\n\n**Limitations**: Synchronous (blocking for large data), not available in **Web Workers**, shared across all scripts on the same origin (security risk if running third-party scripts), not suitable for sensitive data (visible in DevTools and accessible by any script on the page).\n\n### Advanced Storage\n\nFor more advanced storage: **IndexedDB** is asynchronous and can handle large structured datasets; **cookies** can be sent with server requests and support expiry and \`HttpOnly\` flag; [Cache API](https://developer.mozilla.org/en-US/docs/Web/API/Cache) is designed for service workers and *offline-first* apps.`,
...
const cart = storage.get("cart", []);`,
    ...
    },
    {
      ...
      content: `**ES6** (ES2015) was a revolutionary update to JavaScript. Here are the most important modern features you should use daily.\n\n### Template Literals & Destructuring\n\n**Template Literals** use backticks for string interpolation, multi-line strings, and embedded expressions: \`Hello \${user.name}!\`. [Destructuring](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/Destructuring_assignment) extracts values from objects and arrays cleanly: \`const { name, age } = user\` and \`const [first, ...rest] = array\`. Works in function parameters too: \`function render({ title, body })\`.\n\n### Spread & Rest\n\n**Spread** (\`...\`) expands iterables: \`[...arr1, ...arr2]\` merges arrays, \`{ ...obj, key: value }\` clones/extends objects. **Rest** (\`...\`) collects remaining values into an array. **Default parameters**: \`function greet(name = "World")\` set fallback values.\n\n### Optional Chaining & Nullish Coalescing\n\n[\`?.\`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/Optional_chaining) (**Optional Chaining**) safely navigates nested properties: \`user?.address?.city\` returns \`undefined\` instead of crashing. [\`??\`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/Nullish_coalescing) (**Nullish Coalescing**) provides a default only for \`null\`/\`undefined\`: \`value ?? "default"\` (unlike \`||\`, which also replaces \`0\` and \`""\`).\n\n### Modules\n\n**Modules**: \`import\`/\`export\` organize code into reusable files. **Named exports** allow multiple per file; **default export** is one per file. Dynamic imports \`import()\` enable **code splitting** — load modules lazily at runtime.\n\n### Other Essentials\n\nOther essentials: [\`Map\`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Map)/[\`Set\`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Set) collections with unique keys, \`for...of\` loops for iterables, \`Symbol\` for unique property keys, **generator functions** (\`function*\`), \`WeakMap\`/\`WeakRef\` for memory-efficient references.`,
...
const isURL = /^https?:\\/\\/.+/.test(input);`,
    ...
    },
    {
      ...
      content: `Robust error handling separates production-quality code from prototypes. JavaScript errors come in two flavors: **synchronous** (thrown in regular code) and **asynchronous** (rejected Promises or failed \`fetch\` calls).\n\n### try/catch/finally\n\nThe [\`try\`/\`catch\`/\`finally\`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/try...catch) block: The \`try\` block runs code that might throw. The \`catch\` block receives the \`Error\` object (with \`message\`, \`name\`, \`stack\` properties). The \`finally\` block ALWAYS runs — use it for cleanup (*hide spinners, release resources*). Re-throw unexpected errors to not silently swallow them.\n\n### Custom Error Classes\n\n**Custom Error classes** extend the built-in \`Error\`: \`class ValidationError extends Error\` makes error handling meaningful. Use \`instanceof\` in \`catch\` to respond differently to different error types.\n\n### Async Error Handling\n\nAlways wrap \`await\` calls in \`try\`/\`catch\`. Handle rejected Promises from \`Promise.all\` — if one rejects, all reject. Use \`Promise.allSettled\` to handle individual failures. **Unhandled Promise rejections** cause warnings (and crashes in **Node.js**) — always attach \`.catch()\` or use \`try\`/\`catch\`.\n\n### Debugging Tools\n\n\`console.log\` is basic. \`console.table()\` renders arrays as formatted tables. \`console.group()\`/\`groupEnd()\` nests related logs. \`console.time()\`/\`timeEnd()\` measures performance. The \`debugger\` statement pauses execution and opens **DevTools**. **Breakpoints** in DevTools let you step through code, inspect the call stack, and watch variable values. The **Network panel** in DevTools shows all \`fetch\` requests, responses, and timing.`,
...
console.timeEnd("fetch");         // "fetch: 120ms"`,
    ...
    },
    {
      ...
      content: `Writing clean, maintainable JavaScript takes discipline. These practices are observed in professional codebases worldwide.\n\n### Use Strict Mode\n\nUse **strict mode** by adding \`"use strict"\` at the top of files. It prevents silent errors (assigning undeclared variables) and disables confusing legacy features. **ES modules** use strict mode automatically.\n\n### Prefer Immutability\n\nUse \`const\` by default. When working with arrays and objects, use methods that return new values (\`map\`, \`filter\`, spread) rather than mutating in place. **Immutable data** is easier to reason about, debug, and test.\n\n### Write Small, Pure Functions\n\nEach function should do **ONE thing**, have a descriptive name, take explicit inputs, and return predictable outputs with no side effects. **Pure functions** are trivially testable. Prefer \`getActiveUsers()\` over \`process()\`.\n\n### Handle Edge Cases Defensively\n\nWhat happens with \`null\`, \`undefined\`, empty arrays, or zero? Use **optional chaining** (\`?.\`), **nullish coalescing** (\`??\`), and input validation at system boundaries.\n\n### Organize with Modules\n\nOne file per concern: \`api.js\` for HTTP, \`utils.js\` for helpers, \`constants.js\` for fixed values. Avoid **global variables** — they create hidden coupling.\n\n### Write Asynchronous Code Correctly\n\nAlways \`await\` or return Promises. Avoid mixing \`.then()\` chains and \`async\`/\`await\` in the same flow. Handle all rejections — **unhandled rejections** crash Node apps and produce console warnings in browsers.`,
      keyPoints: [
    ...
    { title: 'Best Practices & Code Standards', content: `Use \`const\` by default, \`let\` only when reassignment is needed, and **never** \`var\` (it has function scope and hoisting bugs). Always use **strict equality** (\`===\`) to avoid JavaScript's surprising type coercion rules.\n\n### Error Handling\n\n**Handle all errors explicitly.** Every \`async/await\` call should be wrapped in \`try/catch\`. Every \`.then()\` chain needs a \`.catch()\`. Unhandled promise rejections crash Node.js and silently fail in browsers. Use **optional chaining** (\`?.\`) and **nullish coalescing** (\`??\`) for safe property access.\n\n### Modern Patterns\n\nPrefer \`async/await\` over nested callbacks or raw promises for readability. Use **destructuring**, **template literals**, and **array methods** (\`map\`, \`filter\`, \`reduce\`) instead of manual loops. Keep functions small — if a function does more than one thing, split it.`, code: `// DO: const by default, let only when needed\nconst API_URL = '/api/users';\nlet currentPage = 1;\n\n// DON'T: var (hoisting bugs, no block scope)\nvar data = [];\n\n// DO: Strict equality\nif (value === null) { }\nif (typeof x === 'undefined') { }\n\n// DON'T: Loose equality (type coercion surprises)\n// 0 == "" is true, null == undefined is true\nif (value == null) { }\n\n// DO: Optional chaining + nullish coalescing\nconst city = user?.address?.city ?? 'Unknown';\n\n// DON'T: Verbose null checks\nconst city2 = user && user.address ? user.address.city : 'Unknown';\n\n// DO: async/await with error handling\nasync function fetchUsers() {\n  try {\n    const res = await fetch('/api/users');\n    if (!res.ok) throw new Error('HTTP ' + res.status);\n    return await res.json();\n  } catch (err) {\n    console.error('Fetch failed:', err.message);\n    return [];\n  }\n}\n\n// DON'T: Unhandled promise\nfetch('/api/users').then(r => r.json()).then(d => setData(d));`, codeLabel: 'Dos & Don\'ts', keyPoints: ['Use const by default, let only when reassignment is needed, never var.', 'Always use strict equality (===) to avoid type coercion bugs.', 'Handle all async errors with try/catch or .catch().', 'Use optional chaining (?.) and nullish coalescing (??) for safe access.'] }
  ]
};
