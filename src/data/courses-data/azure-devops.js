...
[Microsoft Learn — Azure Fundamentals](https://learn.microsoft.com/en-us/training/paths/az-900-describe-cloud-concepts/) — Free, structured learning paths with hands-on labs. [Azure DevOps Documentation](https://learn.microsoft.com/en-us/azure/devops/) — Official comprehensive documentation. [Terraform Azure Provider Docs](https://registry.terraform.io/providers/hashicorp/azurerm/latest/docs) — Reference for Terraform with Azure.`,
    ...
    { title: 'Best Practices & Code Standards', content: `Define all pipelines as **YAML code** stored in version control — never use the classic editor for production pipelines. YAML pipelines are reviewable, auditable, and reproducible. Use **templates** to share common pipeline logic across repositories and avoid duplication.\n\n### Secrets & Security\n\n**Never** store secrets, passwords, or API keys in pipeline YAML files. Use **Variable Groups** linked to **Azure Key Vault** for secret management. Apply the principle of least privilege — pipeline service connections should have only the permissions they need.\n\n### Environments & Quality Gates\n\nUse **Environments** with **approval gates** for production deployments. Enforce **branch policies** on protected branches: require PR reviews, passing CI builds, linked work items, and resolved comments before merging. Every merge to main should produce a tested, deployable artifact through an automated pipeline.`, code: `# DO: Pipeline as code with templates\ntrigger:\n  branches:\n    include: [main, release/*]\n\nvariables:\n  - group: production-secrets  # Linked to Key Vault\n\nstages:\n  - stage: Build\n    jobs:\n      - job: BuildAndTest\n        pool:\n          vmImage: 'ubuntu-latest'\n        steps:\n          - script: dotnet build --configuration Release\n          - script: dotnet test --no-build --logger trx\n          - task: PublishTestResults@2\n            inputs:\n              testResultsFormat: 'VSTest'\n              testResultsFiles: '**/*.trx'\n\n  - stage: Deploy\n    dependsOn: Build\n    condition: succeeded()\n    jobs:\n      - deployment: Production\n        environment: 'production'  # Requires approval\n        strategy:\n          runOnce:\n            deploy:\n              steps:\n                - script: echo "Deploying to production"\n\n# DON'T: Store secrets in YAML\n# variables:\n#   DB_PASSWORD: 'my-secret-password'  # NEVER!`, codeLabel: 'Pipeline Best Practices', keyPoints: ['Define all pipelines as YAML in version control — never use classic editor.', 'Never store secrets in YAML — use Variable Groups linked to Key Vault.', 'Use Environments with approval gates for production deployments.', 'Enforce branch policies: require PR reviews and passing CI before merge.'] }
  ]
};
