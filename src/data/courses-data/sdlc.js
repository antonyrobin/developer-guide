  ...
  sections: [
    {
      ...
      image: '/images/sdlc/sdlc-software-types.jpeg',
      keyPoints: [
    ...
    },
    {
      ...
      content: `SDLC has six main phases, like chapters in a story. Each builds on the last, creating a pipeline from idea to deployed product.\n\nPhase 1 — Requirement Analysis: Gather what users want (e.g., "I need an app to track fitness"). Interview stakeholders and document needs. Use tools like surveys or mind maps. Goal: Avoid building the wrong thing!\n\nPhase 2 — Design: Plan the blueprint — how the app looks (UI), stores data (database), and works (architecture). It's like sketching a house before building it. High-Level Design (HLD) defines the overall system, while Low-Level Design (LLD) details individual modules. Tools like Figma help with UI design.\n\nPhase 3 — Development (Coding): Developers write actual code, turning designs into working software. The design documents guide the team, and coding standards ensure consistency. Version control (Git), code reviews, and pair programming maintain quality.\n\nPhase 4 — Testing: Check for bugs. Test functionality, speed, and security. It's like taste-testing your cake before serving. This includes Unit Testing (individual components), Integration Testing (modules together), System Testing (full system), and User Acceptance Testing (UAT). Tools like Selenium automate this.\n\nPhase 5 — Deployment: Launch the software! Put it on servers or app stores for users. This can be a Big Bang deployment (all at once) or a phased rollout (canary, blue-green). Learn cloud platforms like AWS for easy deployment.\n\nPhase 6 — Maintenance: Fix issues post-launch, add features, or update for new tech. Like servicing your car after buying it. This phase never ends — software evolves! It often consumes the majority of a project's total lifetime cost.`,
      image: '/images/sdlc/sdlc-phases.png',
      keyPoints: [
    ...
    },
    {
      ...
      content: `The Waterfall Model is the oldest and most straightforward SDLC model. It follows a strict linear sequential flow — finish one phase fully before the next, like a waterfall that cannot flow upward. Each phase produces a formal document that serves as the input for the next.\n\nThe phases in Waterfall are: Requirements → Design → Implementation → Testing → Deployment → Maintenance. There is no going back to a previous phase once it is finished.\n\nPros: Simple for small projects with fixed needs. Produces extensive documentation at each phase. Good for projects with strict regulatory compliance where documentation is mandatory.\n\nCons: Hard to change midway. If a requirement is misunderstood during the analysis phase, the error may not be discovered until the testing phase — by which time fixing it is extremely expensive. There is no opportunity for customer feedback until the very end.\n\nExample: Building a basic website with set features, government contracts, or embedded systems with hardware dependencies.\n\nDespite its limitations, Waterfall taught the industry the importance of documentation, planning, and structured phases. Many modern methodologies evolved as a direct response to Waterfall's weaknesses. Start with Waterfall for personal projects to understand structured development.`,
      image: '/images/sdlc/sdlc-waterfall-model.jpeg',
      keyPoints: [
    ...
    },
    {
      ...
      content: `Agile is a flexible way to build software, focusing on small, quick deliveries instead of one big launch. It's like eating a pizza slice by slice — enjoy progress along the way. Agile is defined by the Agile Manifesto (2001) which emphasizes: Individuals and interactions over processes and tools, Working software over comprehensive documentation, Customer collaboration over contract negotiation, and Responding to change over following a plan.\n\nKey Characteristics:\n\nShort Cycles — Work in "sprints" (short bursts of 1-4 weeks).\nFrequent Releases — Deliver working parts often.\nCustomer Feedback — Involve users early and often.\nAdaptability — Change plans if needs evolve.\nTeam Collaboration — Daily meetings (stand-ups) keep everyone aligned.\n\nAgile assumes that requirements will change. Instead of fighting change, Agile embraces it. The backlog — a prioritized list of features and tasks — is constantly refined. Items at the top are well-defined and ready for development; items at the bottom are rough ideas that will be refined later.\n\nKey Agile frameworks include Scrum (the most popular, with defined roles, events, and artifacts), Kanban (continuous flow without fixed sprints), Extreme Programming (XP, focused on technical practices like TDD and pair programming), and SAFe (Scaled Agile Framework for large enterprises). It's beginner-friendly because you learn from mistakes quickly and is popular in companies like Google or Spotify.`,
      image: '/images/sdlc/sdlc-agile-overview.png',
      keyPoints: [
    ...
    },
    {
      ...
      content: `In Agile, success depends on clear roles within the team. Unlike traditional setups with strict hierarchies, Agile promotes collaboration. Think of it as a sports team: everyone has a position, but they work together to win.\n\nProduct Owner (PO) — Represents the customer or stakeholders. They prioritize the "product backlog" (a list of features and tasks), define user stories (e.g., "As a user, I want to log in securely"), and ensure the team builds the most valuable features first. Like a movie director — decides the story and vision, but doesn't act in every scene. The PO must be available to clarify requirements and make decisions quickly. A PO who is absent or indecisive creates a bottleneck for the entire team.\n\nScrum Master (SM) — Acts as a coach or facilitator. They remove obstacles (e.g., tool issues or team conflicts), enforce Agile practices, run meetings like daily stand-ups, and help the team improve. Like a referee in a game — ensures fair play and smooth flow without playing themselves. A great Scrum Master makes themselves unnecessary over time by building a self-organizing team.\n\nDevelopment Team — The hands-on builders: developers, designers, testers, etc. They self-organize to complete sprint tasks, collaborate daily, and deliver working software. No titles like "lead developer" matter in Scrum — the whole team is accountable. Like the players on the field — working together to score goals (deliver features). Teams are cross-functional (mix of skills) and small (5-9 people) for efficiency.\n\nIn scaled environments, additional roles may appear: Release Train Engineer (SAFe), Agile Coach, and Chapter Lead (Spotify model). These roles help coordinate across multiple teams while preserving team autonomy.`,
      keyPoints: [
    ...
    },
    {
      ...
      content: `Many companies mix both approaches — use Traditional for planning, Agile for execution. Here's a comparison of the two approaches across key aspects:\n\nApproach — Traditional SDLC (e.g., Waterfall) follows a sequential process (one phase at a time), while Agile follows an iterative approach (loops of improvement).\n\nFlexibility — Traditional has low flexibility (hard to change requirements once a phase is complete), while Agile has high flexibility (adapts to changes easily at any point).\n\nFeedback — In Traditional, feedback comes at the end of the project when the product is delivered. In Agile, feedback is continuous — gathered after each sprint.\n\nDelivery — Traditional aims for one big release at the end. Agile delivers frequent small releases, each adding value.\n\nRisk Management — Traditional carries higher risk if requirements change mid-project. Agile has lower risk due to early and continuous feedback loops.\n\nBest For — Traditional works best for fixed, simple projects with stable requirements. Agile excels in dynamic, complex projects where requirements evolve.\n\nDocumentation — Traditional produces extensive documentation at every phase. Agile favors working software over comprehensive documentation, though essential docs are still maintained.\n\nIn practice, many organizations use a hybrid approach — following Agile for software development while using Waterfall-style stages for procurement and compliance. The key is to understand the principles behind each model and apply them thoughtfully rather than dogmatically.`,
