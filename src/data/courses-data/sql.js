...
GROUP BY order_date::date;`,
      ...
      { title: 'Best Practices & Code Standards', content: `**Create indexes** on columns used in \`WHERE\`, \`JOIN\`, and \`ORDER BY\` clauses — missing indexes are the most common cause of slow queries. But don't over-index: each index slows down \`INSERT\`/\`UPDATE\` operations and consumes storage.\n\n### Query Safety\n\n**Never** use \`SELECT *\` in production code — specify only the columns you need. This reduces data transfer, improves cache utilization, and prevents breaking changes when new columns are added. **Always** use parameterized queries to prevent SQL injection — never concatenate user input into SQL strings.\n\n### Optimization\n\nUse \`EXPLAIN ANALYZE\` to understand query execution plans and identify missing indexes or full table scans. Normalize your schema first (3NF), then denormalize only when you have **measured** evidence of a performance problem. Use appropriate data types — \`INT\` instead of \`VARCHAR\` for numeric IDs, \`TIMESTAMP\` instead of \`VARCHAR\` for dates.`, code: `-- DO: Create indexes on frequently queried columns\nCREATE INDEX idx_users_email ON users(email);\nCREATE INDEX idx_orders_user_date ON orders(user_id, created_at);\n\nSELECT id, name, email FROM users WHERE email = ?;\n\n-- DON'T: Select everything without indexes\nSELECT * FROM users WHERE email = 'test@mail.com';\n\n-- DO: Parameterized queries (prevent SQL injection)\n-- Python: cursor.execute("SELECT * FROM users WHERE id = %s", (user_id,))\n-- Java:   ps = conn.prepareStatement("SELECT * FROM users WHERE id = ?")\n-- C#:     command.Parameters.AddWithValue("@id", userId)\n\n-- DON'T: String concatenation (SQL injection!)\n-- "SELECT * FROM users WHERE id = " + userId\n\n-- DO: Use JOINs efficiently\nSELECT o.id, o.total, u.name\nFROM orders o\nINNER JOIN users u ON o.user_id = u.id\nWHERE o.status = 'completed';\n\n-- DO: Use EXPLAIN to analyze query performance\nEXPLAIN ANALYZE\nSELECT * FROM orders WHERE user_id = 42;\n\n-- DO: Use appropriate constraints\nALTER TABLE users ADD CONSTRAINT uq_email UNIQUE (email);\nALTER TABLE orders ADD CONSTRAINT fk_user\n    FOREIGN KEY (user_id) REFERENCES users(id);`, codeLabel: 'Dos & Don\'ts', keyPoints: ['Create indexes on columns used in WHERE, JOIN, and ORDER BY.', 'Never use SELECT * in production — specify only needed columns.', 'Always use parameterized queries to prevent SQL injection.', 'Use EXPLAIN ANALYZE to identify slow queries and missing indexes.'] }
    ]
  };
